...
<div class="font-['M_PLUS_1p'] min-h-screen bg-cover bg-center bg-no-repeat py-16 px-4" style="background-image: url('{{ asset('images/flower.avif') }}');">
    ...
    <div class="text-center mb-16">
        ...
        <h2 class="text-2xl md:text-3xl font-bold text-white tracking-wide">ようこそ、管理者 {{ Auth::user()->name }} 様</h2>
    </div>

    {{-- メインカード（グリッド2列） --}}
    <div class="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
        {{-- カード1：メニュー一覧 --}}
        <a href="{{ route('admin.menus.index') }}" class="no-underline text-current flex items-start gap-4 p-6 bg-white/60 rounded-2xl shadow-xl hover:scale-105 hover:shadow-2xl transition-transform duration-300">
            <img src="{{ asset('images/logo-square.svg') }}" alt="Icon" class="h-8 w-8">
            <div>
                <h3 class="text-lg font-bold text-gray-800 mb-1">メニュー一覧を見る</h3>
                <p class="text-sm text-gray-600">サービス内容の確認・管理</p>
            </div>
        </a>
        ...
        {{-- カード3：予約一覧 --}}
        <a href="{{ route('admin.reservations.index') }}" class="no-underline text-current flex items-start gap-4 p-6 bg-white/60  rounded-2xl shadow-xl hover:scale-105 hover:shadow-2xl transition-transform duration-300">
            <img src="{{ asset('images/logo-square.svg') }}" alt="Icon" class="h-8 w-8">
            <div>
                <h3 class="text-lg font-bold text-gray-800 mb-1">予約一覧を確認</h3>
                <p class="text-sm text-gray-600">予約状況の管理・確認</p>
            </div>
        </a>

        {{-- カード4：スタッフ一覧 --}}
        <a href="{{ route('staff.index') }}" class="no-underline text-current flex items-start gap-4 p-6 bg-white/60 rounded-2xl shadow-xl hover:scale-105 hover:shadow-2xl transition-transform duration-300">
            <img src="{{ asset('images/logo-square.svg') }}" alt="Icon" class="h-8 w-8">
            <div>
                <h3 class="text-lg font-bold text-gray-800 mb-1">スタッフ一覧</h3>
                <p class="text-sm text-gray-600">スタッフ情報の確認・編集</p>
            </div>
        </a>
    </div>
