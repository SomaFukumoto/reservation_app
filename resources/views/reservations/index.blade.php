                ...
                @foreach ($reservations as $reservation)
                    <div class="bg-white/70  p-6 rounded-xl shadow-md flex flex-col sm:flex-row sm:justify-between sm:items-center">
                        <div class="mb-4 sm:mb-0">
                            <p class="font-semibold text-gray-800">{{ $reservation->menu->name }}</p>
                            <p class="text-sm text-gray-600">スタッフ: {{ $reservation->staff->name ?? '（未指名）' }}</p>
                            <p class="text-sm text-gray-600">日時: {{ \Carbon\Carbon::parse($reservation->reservation_at)->format('Y年m月d日 H:i') }}</p>
                        </div>
                        <div>
                            <span class="px-3 py-1 rounded-full text-sm font-medium
                                @if ($reservation->status === 'canceled') bg-red-100 text-red-700
